...
# For libclang (required by bindgen)
...
```bash
sudo dnf install fontconfig-devel clang-devel
```

**macOS:**

```bash
brew install fontconfig llvm
```

**Windows:**

- Install LLVM from https://releases.llvm.org/ or via `winget install LLVM.LLVM`
...
Analyzes compression and file organization patterns across Reloaded-II mod archives (~2500 mods). Useful for understanding real-world mod distribution patterns.

**Sample Usage:**

```bash
cd mod-archive-stats/make-mod-stats
cargo run --release
```

**Requirements:**

- 7-Zip CLI in PATH (`7z`)
