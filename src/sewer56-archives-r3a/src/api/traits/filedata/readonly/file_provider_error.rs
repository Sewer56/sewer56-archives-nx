...
pub enum FileProviderError {
    ...
    /// Failed to decompress R3A compressed data when sourcing from another R3A file.
    #[error("Failed to decompress R3A compressed data when sourcing from another R3A file.")]
    R3ADecompressionError(#[from] R3ADecompressionError),
