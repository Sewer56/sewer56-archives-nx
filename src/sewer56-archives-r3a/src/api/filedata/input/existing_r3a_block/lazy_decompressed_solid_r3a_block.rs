...
/// Represents a block of data from an existing R3A archive that is lazily decompressed
/// on demand.
///
/// This is used when you only want some files from an exising block in a
/// foreign R3A archive. Namely, you have a SOLID block, but only need to lift out some
/// files from it.
///
/// # Usage
///
/// Usage is as follows.
///
/// 1. Create a new [`LazyDecompressedSolidR3ABlock`] instance
/// 2. Call [`LazyDecompressedSolidR3ABlock::consider_file`] for each file you want to extract.
/// 3. Call [`LazyDecompressedSolidR3ABlock::get_data`] to get the decompressed data.
...
pub struct LazyDecompressedSolidR3ABlock {
    /// Raw decompressed data, lazily initialized when needed
    data: OnceCell<Box<[u8]>>,

    /// Provides access to the original R3A archive
    source_r3a_data_provider: Arc<dyn InputDataProvider + Send + Sync>,

    /// Offset of the block in original R3A archive (via `source_r3a_data_provider`)
    block_offset: u64,

    /// Length of the block in the original R3A archive (via `source_r3a_data_provider`)
    ...
    /// Compression used with the original R3A archive (via `source_r3a_data_provider`)
...
unsafe impl Send for LazyDecompressedSolidR3ABlock {}
unsafe impl Sync for LazyDecompressedSolidR3ABlock {}

impl LazyDecompressedSolidR3ABlock {
    /// Creates a new [`LazyDecompressedSolidR3ABlock`]
    ///
    /// # Arguments
    /// * `source_r3a_data_provider` - Provides raw access to the original R3A file
...
mod tests {
    ...
    fn data_sharing_works() {
        ...
        let block = Arc::new(LazyDecompressedSolidR3ABlock::new(
            provider,
            0,
            100,
