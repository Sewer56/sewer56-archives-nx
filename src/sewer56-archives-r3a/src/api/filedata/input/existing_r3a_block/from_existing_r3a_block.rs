use super::LazyDecompressedSolidR3ABlock;
use crate::{api::traits::*, headers::managed::FileEntry};
use crate::{prelude::*, unsize_box2};
use alloc::sync::Arc;

/// This provider allows you to read a file from an existing SOLID R3A block,
/// where the R3A block is lazy decompressed and provided by [`LazyDecompressedSolidR3ABlock`].
///
...
pub struct FromExistingR3ABlock {
    block: Arc<LazyDecompressedSolidR3ABlock>,
    block_offset: u32,
    file_size: u32,
}

impl FromExistingR3ABlock {
    pub fn new(block: Arc<LazyDecompressedSolidR3ABlock>, entry: &FileEntry) -> Self {
        Self {
            block,
...
impl InputDataProvider for FromExistingR3ABlock {
    fn get_file_data<'a>(
        &'a self,
        start: u64,
        length: u64,
        ...
        Ok(unsize_box2!(Box::new(DecompressedR3ABlockFileData::new(
            base_data_ptr
        ))))
    }
}

pub struct DecompressedR3ABlockFileData<'a> {
    /// Raw pointer to the data
    data: &'a [u8],
}

impl<'a> DecompressedR3ABlockFileData<'a> {
    pub fn new(data: &'a [u8]) -> Self {
        Self { data }
    }
}

impl ReadOnlyFileData for DecompressedR3ABlockFileData<'_> {
    fn data(&self) -> &[u8] {
        self.data
    }
}
...
mod tests {
    ...
    fn create_test_block(data: Vec<u8>) -> Arc<LazyDecompressedSolidR3ABlock> {
        let len = data.len();
        let provider = Arc::new(FromStreamProvider::new(Cursor::new(data)));
        let block =
            LazyDecompressedSolidR3ABlock::new(provider, 0, len as u32, CompressionPreference::Copy);
    ...
    fn can_create_provider() {
        ...
        let provider = FromExistingR3ABlock::new(block, &entry);
    ...
    fn multiple_providers_share_block() {
        ...
        let provider1 = FromExistingR3ABlock::new(Arc::clone(&block), &entry1);
        let provider2 = FromExistingR3ABlock::new(block, &entry2);
