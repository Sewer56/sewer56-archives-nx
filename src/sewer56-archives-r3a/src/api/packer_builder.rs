...
/// A builder pattern implementation for creating R3A archives.
///
...
impl<'a> R3APackerBuilder<'a> {
    ...
    /// # Returns
    ///
    /// Returns a new `R3APackerBuilder` instance with default settings initialized.
    pub fn new() -> Self {
        Self {
    ...
    /// # Returns
    ///
    /// Returns a new `R3APackerBuilder` instance with externally set settings.
    ...
    /// This is a convenience method that combines [`R3APackerBuilder::new`] and [`R3APackerBuilder::with_preset`].
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Returns a new [R3APackerBuilder] instance with the specified preset applied.
...
impl Default for R3APackerBuilder<'_> {
    fn default() -> Self {
        Self::new()
    }
}
...
pub enum PackerPreset {
    ...
    /// of an individual R3A archive into separate blocks on a CDN server; reducing
    /// redundant data.
    ///
    /// # Settings
    ///
...
mod tests {
    ...
    fn can_configure_solid_size() {
        let builder = R3APackerBuilder::new().with_solid_size(65536);
        assert_eq!(builder.settings.solid_size, 65536); // not yet sanitized
    }

    #[test]
    fn can_configure_chunk_size() {
        let builder = R3APackerBuilder::new().with_chunk_size(4194304);
    ...
    fn default_creates_new_instance() {
        let builder = R3APackerBuilder::default();
        assert!(!builder.settings.enable_chunked_deduplication);
        assert!(builder.settings.enable_solid_deduplication);
    }
    ...
    fn can_enable_per_extension_dictionary() {
        let builder = R3APackerBuilder::new().with_per_extension_dictionary(true);
    ...
    fn archival_preset_sets_correct_values() {
        let builder = R3APackerBuilder::new().with_preset(PackerPreset::LocalArchivalSSD);
    ...
    fn low_latency_vfs_preset_sets_correct_values() {
        let builder = R3APackerBuilder::new().with_preset(PackerPreset::LowLatencyVFS);
