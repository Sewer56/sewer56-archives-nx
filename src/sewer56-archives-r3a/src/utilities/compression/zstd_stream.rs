...
use super::{DecompressionResult, R3ADecompressionError};
use core::ffi::c_void;
...
impl<'a> ZstdDecompressor<'a> {
    ...
    /// * `Err(R3ADecompressionError)` if the decompressor couldn't be created (usually due to memory allocation failure)
    pub fn new(source: &'a [u8]) -> Result<Self, R3ADecompressionError> {
        unsafe {
    ...
    /// * `Err(R3ADecompressionError)` if an error occurred during decompression
    pub fn decompress_chunk(&mut self, destination: &mut [u8]) -> DecompressionResult {
        ...
        unsafe {
            ...
            if ZSTD_isError(result) != 0 {
                return Err(R3ADecompressionError::ZStandard(ZSTD_getErrorCode(result)));
            }
        }
