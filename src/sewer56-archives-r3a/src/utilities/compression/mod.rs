//! Compression utilities for the R3A archive format.
//!
...
//! The [`R3ACompressionError`] and [`R3ADecompressionError`] enums contain high-level
//! validation errors that are common across all compression algorithms:
//! - [`R3ACompressionError::DestinationTooSmall`]: Destination buffer too small for compression
//! - [`R3ADecompressionError::MaxBlockSizeNotProvided`]: Max block size required for partial decompression
//! - [`R3ADecompressionError::MaxBlockSizeTooSmall`]: Max block size insufficient for destination buffer
//!
...
//! [`R3ACompressionError`]: crate::utilities::compression::R3ACompressionError
//! [`R3ADecompressionError`]: crate::utilities::compression::R3ADecompressionError
//! [`R3ACompressionError::DestinationTooSmall`]: crate::utilities::compression::R3ACompressionError::DestinationTooSmall
//! [`R3ADecompressionError::MaxBlockSizeNotProvided`]: crate::utilities::compression::R3ADecompressionError::MaxBlockSizeNotProvided
//! [`R3ADecompressionError::MaxBlockSizeTooSmall`]: crate::utilities::compression::R3ADecompressionError::MaxBlockSizeTooSmall
...
pub type CompressionResult = Result<usize, R3ACompressionError>;
...
/// compression algorithms in the R3A system.
#[derive(Debug, PartialEq, Eq, Clone, Copy, Error)]
pub enum R3ACompressionError {
...
pub type DecompressionResult = Result<usize, R3ADecompressionError>;

/// Represents an error returned from the R3A decompression APIs.
///
/// This enum contains both library-specific errors (which wrap raw errors from underlying
/// compression libraries) and high-level validation errors that are common across all
/// decompression algorithms in the R3A system.
#[derive(Debug, PartialEq, Eq, Clone, Copy, Error)]
pub enum R3ADecompressionError {
    #[error(transparent)]
...
pub fn compress(
    ...
    *used_copy = false;
    match method {
        ...
        #[cfg(not(feature = "lz4"))]
        CompressionPreference::Lz4 => Err(R3ACompressionError::Lz4NotEnabled),
        #[cfg(feature = "bzip3")]
        CompressionPreference::Bzip3 => bzip3::compress(source, destination, used_copy),
        #[cfg(not(feature = "bzip3"))]
        CompressionPreference::Bzip3 => Err(R3ACompressionError::Bzip3NotEnabled),
        CompressionPreference::NoPreference => {
            zstd::compress(level, source, destination, used_copy)
        }
        CompressionPreference::LZMA => Err(R3ACompressionError::LzmaNotEnabled),
    }
}
...
mod tests {
    ...
    #[rstest]
    #[case::copy(CompressionPreference::Copy, R3ACompressionError::DestinationTooSmall)]
    #[case::zstd(
        CompressionPreference::ZStandard,
        R3ACompressionError::DestinationTooSmall
    )]
    #[cfg_attr(
        feature = "lz4",
        case::lz4(CompressionPreference::Lz4, R3ACompressionError::DestinationTooSmall)
    )]
    #[cfg_attr(
        feature = "bzip3",
        case::bzip3(CompressionPreference::Bzip3, R3ACompressionError::DestinationTooSmall)
    )]
    #[cfg_attr(miri, ignore)]
    fn destination_too_small_returns_err(
        #[case] method: CompressionPreference,
        #[case] expected_compression_error: R3ACompressionError,
    ) {
    ...
    #[case::copy(
        CompressionPreference::Copy,
        R3ADecompressionError::Copy(CopyDecompressionError::DestinationTooSmall)
    )]
    #[case::zstd(
        CompressionPreference::ZStandard,
        R3ADecompressionError::ZStandard(ZSTD_ErrorCode::ZSTD_error_dstSize_tooSmall)
    )]
    #[cfg_attr(
        feature = "lz4",
        case::lz4(
            CompressionPreference::Lz4,
            R3ADecompressionError::Lz4(Lz4DecompressionError::DecompressionFailed)
        )
    )]
    #[cfg_attr(
        feature = "bzip3",
        case::bzip3(
            CompressionPreference::Bzip3,
            R3ADecompressionError::Bzip3(Bzip3CompressionError::DataSizeTooSmall)
        )
    )]
    #[cfg_attr(miri, ignore)]
    fn decompress_buffer_too_small_returns_error(
        #[case] method: CompressionPreference,
        #[case] expected_decompression_error: R3ADecompressionError,
    ) {
    ...
    fn can_terminate_early_streamed(#[case] method: CompressionPreference) {
        ...
        assert!(matches!(result, Err(R3ACompressionError::TerminatedStream(x)) if x == error_code));
    }
