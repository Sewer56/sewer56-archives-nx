...
/// Structure for serializing and deserializing the string pool within the R3A Archive format.
///
/// # Type Parameters
///
/// * `ShortAlloc` - Allocator for short lived memory. Think pooled memory and rentals.
/// * `LongAlloc` - Allocator for longer lived memory. Think same lifetime as creating R3A archive.
///
    ...
    StringPool<ShortAlloc, LongAlloc>
{
    ...
    /// * `long_alloc` - Allocator for longer lived memory. Think same lifetime as creating R3A archive creator/unpacker.
    /// * `use_compression` - Whether to compress the string pool.
    ///   This is only set to 'false' in tests to skip non-rust code under 'miri', and benchmarks.
    ///   In actual archive use this is always 'true'.
    ...
    pub fn unpack_v0_with_allocators(
        ...
        let decompressed: Box<[u8], ShortAlloc> = if use_compression {
            ...
            // SAFETY: Don't trust user input; in case R3A is being ran on a server.
...
mod tests {
    ...
    fn unpack_fails_when_decompressed_size_too_short(#[case] format: StringPoolFormat) {
        ...
        assert!(matches!(
            result,
            Err(StringPoolUnpackError::FailedToDecompress(R3ADecompressionError::ZStandard(zstd_error_code)))
