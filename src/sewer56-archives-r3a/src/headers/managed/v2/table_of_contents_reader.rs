...
{
    /// Deserializes the table of contents [R3A v2.x.x format] from a given address and version.
    ///
    /// # Safety
    ///
    /// This function is unsafe because it works with raw pointers.
...
/// Deserializes the file entries [R3A v2.x.x format] which uses a preset
...
/// Deserializes the file entries [R3A v2.x.x format] which uses the Flexible Format
