...
bitfield! {
    /// Represents the TOC Header structure for Preset 1.
    ///
    ...
    pub struct Preset1TocHeader(u64);
    impl Debug;
...
impl Preset1TocHeader {
    /// Maximum possible size of the Preset1 TOC Header.
    pub const SIZE_BYTES: usize = 8;

    /// Initializes the Preset1 TOC Header with given data.
    ///
    /// - Sets `IsFlexibleFormat` to `0` as per specification.
    /// - Sets `Preset` to `1` as per specification.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// A new `Preset1TocHeader` instance in little-endian format.
    pub fn new(string_pool_size: u32, block_count: u32, file_count: u32) -> Self {
        let mut header = Preset1TocHeader(0);
    ...
    /// Creates a `Preset1TocHeader` from a raw `u64` value.
    ///
    ...
    /// # Returns
    ///
    /// A new `Preset1TocHeader` instance.
    pub fn from_raw(raw: u64) -> Self {
        Preset1TocHeader(raw.to_le())
    }

    /// Converts the `Preset1TocHeader` to little-endian format.
    pub fn to_le(&self) -> Self {
        Preset1TocHeader(self.0.to_le())
    }
...
impl Default for Preset1TocHeader {
    fn default() -> Self {
        Self::new(0, 0, 0)
    }
}
...
mod tests {
    ...
    fn header_size_is_correct() {
        assert_eq!(size_of::<Preset1TocHeader>(), Preset1TocHeader::SIZE_BYTES);
    }

    #[test]
    fn can_init_max_values() {
        let header = Preset1TocHeader::new(
    ...
    fn default_values_are_sane() {
        let header = Preset1TocHeader::default();
        assert!(!header.get_is_flexible_format());
        assert_eq!(header.get_preset(), 1);
        assert_eq!(header.get_string_pool_size(), 0);
        assert_eq!(header.get_block_count(), 0);
    ...
    fn from_raw_creates_correct_header() {
        let raw: u64 = 0x20123456789ABCDE;
        let header = Preset1TocHeader::from_raw(raw);
